...
import type { InelasticCollisionEq, Value } from '@/types/equations'

// Default width for Object 1 & 2
const [w1, w2] = [1, 1]
const [x1, x2] = [-2.5, 2.5]
export default ():{ equationString:string, values: Value[], equation: () => InelasticCollisionEq } => {
  ...
  const equation = () => {
    ...
    const vp =  (m1_num * v1_num + m2_num * v2_num) / (m1_num + m2_num)
    ...
    return (t:number) => ( 
      t <= collisionTime ? {
        ...
        x1: x1 + v1_num * collisionTime + vp * (t - collisionTime),
        x2: x2 + v2_num * collisionTime + vp * (t - collisionTime)
      })
  }
